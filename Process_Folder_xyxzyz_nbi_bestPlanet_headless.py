...
if(PROCESS_FLAG):
    # for colour_folder in colour_folders:
	for colour_folder in progressbar(colour_folders, "Processing: ", 40)
        print("Colour folder :" + colour_folder)
        try:
            current_file_path = os.path.join(
                colour_folder, "Deconvolved_32bit.raw")
            print("Opening")
            # print(current_file_path)
    ...
    #			z_project_contrast,path_to_z_project = z_projectfile(imp)
            for projection_axis_key in projections_axes:
                ...
                path_to_z_project = os.path.join(
                    colour_folder, projection.getTitle() + "_" + str(projection_axis_key))
