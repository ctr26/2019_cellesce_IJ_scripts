...
def project(imp,axis):

	maxOp = ops.op("stats.max", imp)
	x_dim, y_dim, nChannels, z_dim, nFrames = imp.getDimensions()
	if(axis==0): slicer = [y_dim,z_dim]
#	if(axis==1): slicer = [x_dim,y_dim]
#	if(axis==2): slicer = [x_dim,z_dim]
	if(axis==1): slicer = [x_dim,z_dim]
	if(axis==2): slicer = [x_dim,y_dim]

	projection = ops.create().img(slicer)

	projection_16bit = ops.convert().uint16(projection)
	ops.run("project", projection_16bit, imp, maxOp, axis)
	impProjection = ImageJFunctions.wrap(projection_16bit,"project")	
#	if(SHOW_FLAG):z_project.show()
	project_contrast = ij.plugin.Duplicator().run(impProjection)
	IJ.run(project_contrast, "Enhance Contrast", "saturated=0")
	IJ.run(project_contrast, "Apply LUT", "0.35")
#	print(z_project_contrast)
	return project_contrast
...
dir = 'T:\\DATA0\\LSM\\_Processed\\_2019_kcl\\_processed\\2019-05-15 - Fuad - HipSci\\190515_11.36.30_E8-1\\190515_11.36.30_Step_Size_-0.4_Wavelength_DAPI 452-45_E8-1\\'
...
#			z_project_contrast,path_to_z_project = z_projectfile(imp)
		for projection_axis_key in projections_axes:
			...
			projection = project(imp,projections_axis)
#			print(projection.getTitle())
...
#				 current_file = os.path.join(wavelength,"DUP_project_XY.tif")
...
#	image_count = len(image_folders)
	...
	montaged=MontageMaker.makeMontage2(concat_montage_array,int(montage_columns),int(montage_rows),1.00,1,image_count,1,0,True)
	montaged.show()
