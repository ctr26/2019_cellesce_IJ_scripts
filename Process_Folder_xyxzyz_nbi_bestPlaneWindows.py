...
#	print(slicer)
	...
	if(SHOW_FLAG):impProjection.show()
...
def best_plane(imp):
	...
	ops.run("project", projection, imp, maxOp, axis_1)
	impProjection = ImageJFunctions.wrap(projection,"best_plane")
	
	impProjection_blur = ops.filter().gauss(projection, [blurring_kernel_size, 0, 0])
	...
	profile = ProfilePlot(line_profile_imp).getProfile()
...
dir = 'G:\\My Drive\\_data\\_cellesce\\190507_17.25.26_Step_Size_+0.4_Wavelength_DAPI 452-45_Position_Position_0_ISO49_5FU_10uM\\'
...
#				 current_file = os.path.join(wavelength,"DUP_project_YZ.tif")
				 current_file = os.path.join(wavelength,"project_XY.tif")
